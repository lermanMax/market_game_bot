...
class SuperAdminData:
    def __init__(self, admin_id: int):
        ...
        connection = psycopg2.connect(**db_config)
        with connection.cursor() as cursor:
            select_script = '''SELECT superadmin.tg_id
                                FROM superadmin 
                                WHERE admin_id = %s;'''
    ...
    def get_id_list_of_shares(self, company_id: int = None) -> list:
        ...
        with connection.cursor(cursor_factory=extras.DictCursor) as cursor:
            if company_id:
                ...
                values = (self.gameuser_id, company_id)
            else:
                select_script = '''
                    SELECT share.share_id FROM share
                    WHERE share.owner = %s;'''
                values = (self.gameuser_id,)
    ...
    def get_price(self) -> float:
        ...
        with connection.cursor() as cursor:
            select_script = '''
                        ...
                        FROM company WHERE company_id = %s;'''
            cursor.execute(select_script, (self.company_id,))
...
class GameData:
    ...
    def add_company(
        ...
        with connection.cursor() as cursor:
            insert_values = (
                company_name, company_ticker, price, self.game_id, 0)
            insert_script = '''
                INSERT INTO company
    ...
    def get_list_of_company_ids(self) -> list:
        connection = psycopg2.connect(**db_config)
        with connection.cursor(cursor_factory=extras.DictCursor) as cursor:
            select_script = '''SELECT company.company_id 
                                FROM company 
    ...
    def create_share(company_id: int, owner_gameuser_id: int) -> int:
        ...
        with connection.cursor() as cursor:
            ...
            insert_script = '''INSERT INTO share (company, owner) 
                                VALUES (%s, %s) 
                                RETURNING share_id;'''
            cursor.execute(insert_script, insert_values)
    ...
    def get_transactions(date_deal, type_deal, company_id) -> list:
        ...
        with connection.cursor(cursor_factory=extras.DictCursor) as cursor:
            ...
            select_script = '''SELECT transaction_id, date_deal, subject_deal, 
    ...
    def add_company_history(
        ...
        with connection.cursor() as cursor:
            ...
            insert_script = '''INSERT INTO company_history (company,
                                date_entry, price) 
                                VALUES (%s, %s, %s);'''
            cursor.execute(insert_script, insert_values)
        connection.commit()
...
class ShareData:
    def __init__(self, share_id: int):
        ...
        connection = psycopg2.connect(**db_config)
        with connection.cursor() as cursor:
            select_script = '''SELECT share.company, share.owner 
                                FROM share 
                                WHERE share_id = %s;'''
